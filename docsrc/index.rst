...
An implementation of a uniform B-spline from
:math:`\mathbb{R}^n \rightarrow \mathbb{R}^m`. Uniform means, that
...
Let's start with a B-spline
:math:`f\colon \mathbb{R} \rightarrow \mathbb{R}`.

.. code:: cpp

        ubs::UniformBSpline<double, 3, double, double, std::vector<double>> spline;

The first template argument is the value type, the second one is the
...
template parameter is the container of control points.
...
As we have used the default constructor for our spline, the spline is
...
Now we have defined a spline which is a straight line with a slope of
...
The first statement evaluates the first derivative at zero, the second
the second derivative at one.

Also the smoothness of the spline can be evaluated. For the used
definition of the smoothness see UniformBSpline::smoothness.
...
Now lets look at a spline
:math:`f\colon \mathbb{R} \rightarrow \mathbb{R}^3`.

.. code:: cpp

        ubs::UniformBSpline<double, 3, double, Eigen::Vector3d, EigenAlignedVec<Eigen::Vector3d>> spline;

The definition is very similar to the one dimensional case but the
output is now a Eigen::Vector3d.

As we still have a one dimensional input, spline evaluation, derivatives
and smoothness works similar, the only change is its output type:
...
The smoothness is now divided into each dimension. To get the smoothness
of the whole spline, those values needs to be summed together.

2D |rarr| 1D Eigen spline
~~~~~~~~~~~~~~~~~~~~~~~~~

Now lets look at a spline
:math:`f\colon \mathbb{R}^2 \rightarrow \mathbb{R}`.

.. code:: cpp

        ubs::UniformBSpline<double, 3, Eigen::Vector2d, double, Eigen::MatrixXd> spline;

The input type is now Eigen::Vector2d and the output type is double. As
...
This would evaluate the spline at (0, 0). This can be shortened by using
brace initialization of Eigen::Vector2d:

.. code:: cpp

        val = spline.evaluate({0.0, 0.0});

For the derivatives we now have partial derivatives. One can specify the
derivative for each dimension separately.
...
This calculates the expression
:math:`\frac{\partial^2 f(x, y)}{\partial x \partial y}`.
...
Not lets look at a spline
:math:`f\colon \mathbb{R}^2 \rightarrow \mathbb{R}^3`.
...
So we have used an Eigen::Vector2d as input, an Eigen::Vector3d as
output and a boost::multi\_array as control points container. As the
definition of such splines are getting pretty lengthy, the alias
declaration "EigenUniformBSpline" exists:

.. code:: cpp

        ubs::EigenUniformBSpline<double, 3, 2, 3> splineS;

To evaluate a spline, one need a two dimensional input and receives a
